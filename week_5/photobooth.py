...
# Load image for sift/FLANN detection and matching
hanon_book = cv.imread('hanon.jpg')
...
# SIFT with FLANN Feature Detection and Matching
#  if feature_detection_flag == True:
#   gray_book = cv.cvtColor(hanon_book, cv.COLOR_BGR2GRAY)
#   kp_sift_book, des_sift_book = sift.detectAndCompute(gray_book, None)
#   img_sift_book = cv.drawKeypoints(gray_book, kp_sift_book, hanon_book, flags=cv.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
#
#   gray_frame = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
#   kp_sift_frame, des_sift_frame = sift.detectAndCompute(gray_frame, None)
#   img_sift_frame = cv.drawKeypoints(gray_frame, kp_sift_frame, frame, flags=cv.DrawMatchesFlags_DRAW_RICH_KEYPOINTS)
#
#   flann_sift_matches = flann.knnMatch(des_sift_book, des_sift_frame, k=2)
...
#
#   frame = cv.drawMatchesKnn(gray_book, kp_sift_book, gray_frame, kp_sift_frame, flann_sift_matches, None,
#                                      **draw_params_flann_sift)
 ...
 # elif k == ord('o'):
 #  print('presed o')
 #  feature_detection_flag = not feature_detection_flag
